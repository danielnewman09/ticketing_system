...
Uses call_tool_loop with intermediate tools (validate_design, check_class_name)
so the LLM can self-correct before committing the final design.
...
def design_oo(
    ...
    Uses call_tool_loop with validate_design and check_class_name tools
    so the LLM can self-correct before committing the final design.
    ...
    # Build tool dispatcher with validation context
    ...
    if errors:
        log.warning(
            "design_oo: %d validation issues remain after tool loop for HLR %s: %s",
            len(errors), hlr.get('id', '?'), errors[:5],
        )

    # Warn about empty classes (possible dropped nested arrays)
