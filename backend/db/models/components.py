...
dependency_components = Table(
    ...
    Column(
        "component_id", Integer, ForeignKey("components.id", ondelete="CASCADE"), primary_key=True
    ),
)
...
class Component(Base):
    ...
    parent_id: Mapped[Optional[int]] = mapped_column(
        ForeignKey("components.id", ondelete="CASCADE"), nullable=True
    )
    language_id: Mapped[Optional[int]] = mapped_column(
        ForeignKey("languages.id", ondelete="CASCADE"), nullable=True
    )
    ...
    language: Mapped[Optional[Language]] = relationship(
        "Language", back_populates="environment_components"
    )
...
class Language(Base):
    ...
    environment_components: Mapped[list[Component]] = relationship(
        "Component", back_populates="language"
    )
    build_systems: Mapped[list[BuildSystem]] = relationship(
        "BuildSystem", back_populates="language"
    )
    test_frameworks: Mapped[list[TestFramework]] = relationship(
        "TestFramework", back_populates="language"
    )
    dependency_managers: Mapped[list[DependencyManager]] = relationship(
        "DependencyManager", back_populates="language"
    )
...
class Dependency(Base):
    ...
    __table_args__ = (UniqueConstraint("manager_id", "name", name="unique_dependency_per_manager"),)

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    manager_id: Mapped[int] = mapped_column(
        ForeignKey("dependency_managers.id", ondelete="CASCADE"), nullable=False
    )
    ...
    index_file_patterns: Mapped[str] = mapped_column(
        String(200), default="*.h *.hpp", server_default="*.h *.hpp"
    )
    ...
    manager: Mapped[DependencyManager] = relationship(
        "DependencyManager", back_populates="dependencies"
    )
