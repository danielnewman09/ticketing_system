# Assign Components Prompt Restructure — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the `SYSTEM_PROMPT` string in `assign_components_prompt.py` with the restructured version using HARD-GATE, CONTRACT, FORMAT-CONTRACT, anti-patterns, and guidelines sections.
...
**Files:**
- Modify: `backend/ticketing_agent/design/assign_components_prompt.py` (the `SYSTEM_PROMPT` string)
...
Run: `python -c "from backend.ticketing_agent.design.assign_components_prompt import SYSTEM_PROMPT; print('OK:', len(SYSTEM_PROMPT), 'chars')"`

Expected: `OK: <number> chars` with no import errors.

- [ ] **Step 3: Run existing tests**

Run: `python -m pytest tests/ -x -q --timeout=30 2>&1 | head -30`

Expected: All existing tests pass. The prompt string is a constant — no logic changed, so tests should pass unchanged.

- [ ] **Step 4: Commit**

```bash
git add backend/ticketing_agent/design/assign_components_prompt.py
git commit -m "Restructure assign_components prompt with HARD-GATE, CONTRACT, FORMAT-CONTRACT, and anti-patterns"
```