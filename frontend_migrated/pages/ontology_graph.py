...
Uses the migrated data layer (frontend_migrated.data.ontology) and
codegraph GraphRepository as the single backend.  No imports from
frontend/ or backend/.
...
- Widget calls — delegate all rendering to ``frontend_migrated.widgets``
  helpers (controls, legend, detail panel).
- ``load_graph()`` — fetches graph data from Neo4j via GraphRepository and
