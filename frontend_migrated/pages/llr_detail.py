...
async def llr_detail_page(llr_id: str):
    ...
    """
    apply_theme()
    page_layout(f"LLR {_short_refid(llr_id)}")
    ...
    async def content():
        ...
        hlr = data["hlr"]
        short_llr = _short_refid(data["id"])
        ...
        with ui.row().classes("items-center gap-1 px-2 mt-4"):
            ...
            if hlr:
                short_hlr = _short_refid(hlr["id"])
                ui.link(f"HLR {short_hlr}", f"/hlr/{hlr['id']}").classes(
                    "text-blue-400 text-sm no-underline"
                )
                ui.label("/").classes("text-gray-500 text-sm")
            ui.label(f"LLR {short_llr}").classes("text-sm text-gray-300")

        # Header
        with ui.row().classes("w-full items-center justify-between px-2 mt-2 mb-4"):
            ui.label(f"LLR {short_llr}").classes("text-2xl font-bold")
            ui.button(
                "Back",
                icon="arrow_back",
                on_click=lambda: ui.navigate.to(f"/hlr/{hlr['id']}" if hlr else "/"),
            ).props("flat size=sm")
        ...
        with ui.row().classes("w-full gap-4 px-2 items-start"):
            ...
            with ui.column().classes("flex-1 gap-4"):
                ...
                if hlr:
                    with ui.card().classes("w-full"):
                        ui.label("Parent HLR").classes(CLS_SECTION_HEADER)
                        with ui.row().classes("items-center gap-2"):
                            ui.badge(f"HLR {_short_refid(hlr['id'])}", color="blue").props("outline")
                            if hlr["component"]:
                ...
                if data["components"]:
                    with ui.card().classes("w-full"):
                        ui.label("Components").classes(CLS_SECTION_HEADER)
    ...
    async def show_edit_dialog(current_description: str):
        with ui.dialog() as dialog, ui.card().classes("w-96"):
            ui.label(f"Edit LLR {_short_refid(llr_id)}").classes("text-lg font-bold mb-2")
