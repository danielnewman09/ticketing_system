...
Migrated from ``backend.ticketing_agent.design.scaffold_project`` — no
imports from ``backend/`` anywhere in this module.
"""
...
from backend_migrated.agents.scaffold_project_prompt import build_user_message
...
def scaffold_project(
    ...
    log.info(
        "Phase 2 (build): verifying project builds in %s",
        project_dir,
    )
    log.info(
        "Phase 2 (build): skill=%s, model=%s, max_turns=%d",
        _VERIFY_BUILD_SKILL, model or "(default)", max_turns,
    )
    ...
    try:
        ...
        log.info(
            "Phase 2 complete: build_success=%s, files=%d",
            build_result.get("build_success"),
            len(build_result.get("files_modified", [])),
        )
    except Exception as exc:
        log.error("Phase 2 (build) failed: %s", exc, exc_info=True)
        build_result = {
            "summary": f"Build verification failed: {exc}",
            "files_modified": [],
            "build_success": False,
        }
